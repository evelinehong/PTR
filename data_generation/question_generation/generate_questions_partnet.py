...
def add_empty_filter_options(attribute_map, metadata, num_to_add, template):
  ...
  for a in attribute_map:
    ...
    # for part in parts:
    #   for color in metadata['types']['Color']:
    #     attr_vals.append([obj, part, None, {part:color}])
    #     if part in ["leg", "leg bar", "wheel"] and (not "many" in template['text'][0]) and (not "number" in template['text'][0]):
    #       if obj == "Cart":
    #         amount = random.choice([1,2,3,4])
    #       else:
    #         amount = random.choice([2,3,4,5,6])
    #       attr_vals.append([obj, part, {part: amount}, {part: color}])  
...
# def find_part_filter_options(object_idx, scene_struct, metadata, val):
#   obj = scene_struct['objects'][object_idx]

#   parts = []
#   part_color = obj['part_color_occluded']
#   for part in part_color.keys():
#     if not part in list(val.values):
#       parts.append(part)

#   scene_struct['_parts'] = parts
...
def main(args):
  ...
  for i, scene in tqdm(enumerate(all_scenes)):
    ...
    # if not "000425" in scene_fn: continue
    ...
    # scene_struct["relationships"]["above"] = [[] for l in range (len(scene_struct['objects']))]
    # scene_struct["relationships"]["below"] = [[] for l in range (len(scene_struct['objects']))]
    # pixels = []

    keep = True
    for (i, obj) in enumerate(scene_struct['objects']):
    #   pixels.append(obj['pixel_coords'])
    #   if obj['category'] == 'Refrigerator':
    #     final_part_color = dict()
    #     final_part_mask = dict()
    #     for part, color in obj['part_color'].items():
    #       if part == 'frame': part = 'body'
    #       final_part_color[part] = color
    #     for part, mask in obj['part_mask'].items():
    #       if part == 'frame': part = 'body'
    #       final_part_mask[part] = mask
    #     scene_struct['objects'][i]['part_color'] = final_part_color
    #     scene_struct['objects'][i]['part_mask'] = final_part_mask

    #   obj_name = obj['category']

    #   # if obj_name == 'Table' and (('drawer_box' in scene_struct['objects'][i]['part_color_all'].keys() and scene_struct['objects'][i]['part_color_all']['drawer_box'] == scene_struct['objects'][i]['part_color_all']['frame']) or ('cabinet_door' in scene_struct['objects'][i]['part_color_all'].keys() and scene_struct['objects'][i]['part_color_all']['cabinet_door'] == scene_struct['objects'][i]['part_color_all']['frame'])): keep = False

    #   if obj_name == 'Table' and 'drawer' in scene_struct['objects'][i]['part_count'].keys() and (not 'handle' in scene_struct['objects'][i]['part_color_all'].keys()): keep = False

    #   # if obj_name == 'Table' and 'drawer' in scene_struct['objects'][i]['part_color'].keys() and (not 'leg bar' in scene_struct['objects'][i]['part_color'].keys()): keep = False

    #   # if obj_name == 'Table' and 'drawer' in scene_struct['objects'][i]['part_color'].keys() and (not 'drawer' in scene_struct['objects'][i]['part_mask'].keys()): keep = False

    #   if obj_name == 'Chair':
    #     part_list2 = ['arm', 'leg', 'back', 'seat', 'central support', 'pedestal', 'leg bar', 'wheel', 'arm vertical bar', 'arm horizontal bar']
    #     count_list = ['leg', 'leg bar', 'wheel', 'arm vertical bar', 'arm horizontal bar']
    ...
    #   if obj_name == 'Bed':
    #     part_list2 = ['sleep area', 'back', 'leg', 'leg bar']
    #     count_list = ['leg', 'leg bar']

    #   if obj_name == 'Table': 
    #     part_list2 = ['top', 'drawer', 'door', 'central support', 'leg', 'pedestal', 'shelf', 'leg bar', 'wheel']
    #     count_list = ['leg', 'drawer', 'door', 'leg bar', 'shelf', 'wheel']

    #   if obj_name == 'Cart':
    #     part_list2 = ['body', 'wheel']
    #     count_list = ['wheel']

    #   if "physics" in scene_struct.keys():
    #     part_count_occluded = dict()
    #     part_color_occluded = dict()
    #     # print (question_fn)
    #     for (key, value) in scene_struct['objects'][i]['part_mask'].items():
    #       if not key in count_list: continue
    #       part_count = 0
    #       for v in value:
    #         counts = v["counts"]
    #         count = 0
    ...
    #           part_count += 1
    #       if part_count > 0:
    #         part_count_occluded[key] = part_count
    #         if key in part_list2:
    #           try:
    #             part_color_occluded[key] = obj['part_color'][key]
    #           except:
    #             key2 = key.replace(" ", "_")
    #             part_color_occluded[key] = obj['part_color_all'][key2]

    #     scene_struct['objects'][i]["part_count_occluded"] = part_count_occluded
    #     scene_struct['objects'][i]["part_color_occluded"] = part_color_occluded
      
    #   else:
    #     if keep:
    #       if obj_name == 'Refrigerator' and 'door' in scene_struct['objects'][i]["part_count"].keys():
    #         if scene_struct['objects'][i]["part_count"]['door'] % 2 == 0:
    #           scene_struct['objects'][i]["part_count"]['door'] /= 2
    #           number = scene_struct['objects'][i]["part_count"]['door']
    #           scene_struct['objects'][i]["part_count_occluded"]['door'] = number - (number * 2 - scene_struct['objects'][i]["part_count_occluded"]['door'])
    #       if obj_name == 'Table' and 'drawer' in scene_struct['objects'][i]["part_count"].keys() and (not 'door' in scene_struct['objects'][i]["part_count"].keys()):
    #         if 'handle' in scene_struct['objects'][i]['part_mask'].keys():
    #           scene_struct['objects'][i]["part_count_occluded"]['drawer'] = len(scene_struct['objects'][i]['part_mask']['handle'])
    #         else:
    #           scene_struct['objects'][i]["part_count_occluded"]['drawer'] /= 2
    #       if obj_name == 'Table' and 'drawer' in scene_struct['objects'][i]["part_count"].keys() and  'door' in scene_struct['objects'][i]["part_count"].keys():
    #         if 'handle' in scene_struct['objects'][i]['part_mask'].keys():
    #           scene_struct['objects'][i]["part_count_occluded"]['drawer'] = len(scene_struct['objects'][i]['part_mask']['handle']) - scene_struct['objects'][i]["part_count"]['door']
    #         else:
    #           scene_struct['objects'][i]["part_count_occluded"]['drawer'] /= 2
    #       if 'arm horizontal bar' in scene_struct['objects'][i]["part_count_occluded"].keys():
    #         if scene_struct['objects'][i]["part_count_occluded"]['arm horizontal bar'] > 2:
    #           scene_struct['objects'][i]["part_count_occluded"]['arm horizontal bar'] = 2


    #   # if 'wheel' in obj['part_count_occluded'].keys() and obj['category'] in ['Chair', 'Table']:
    #   #   if "leg" in obj['part_count_occluded'].keys():
    #   #     if scene_struct['objects'][i]['part_count_occluded']['wheel'] == 14:
    #   #       scene_struct['objects'][i]['part_count_occluded']['wheel'] = scene_struct['objects'][i]['part_count_occluded']['leg'] - 1
    #   #     else:
    #   #       scene_struct['objects'][i]['part_count_occluded']['wheel'] = scene_struct['objects'][i]['part_count_occluded']['leg']
          
    #   #   else:
    #   #     keep = False

    #   # if 'door' in obj['part_count_occluded'].keys() and obj['category'] == 'Refrigerator':
    #   #   obj['part_count_occluded']['door'] = obj['part_count']['door'] / 2 - (obj['part_count']['door'] - obj['part_count_occluded']['door'])

    #   if 'physics_mode' in obj.keys() and obj['physics_mode'] == 'support':
    #     support = int(obj['support'][-1]) - 1
    #     for key, value in scene_struct["relationships"].items():
    #       if i in scene_struct["relationships"][key][support]: scene_struct["relationships"][key][support].remove(i)
    #       if support in scene_struct["relationships"][key][i]: scene_struct["relationships"][key][i].remove(support)
    #     scene_struct["relationships"]["above"][support].append(i)
    #     scene_struct["relationships"]["below"][i].append(support)
    ...
    #   left = []
    #   right = []
    #   for (j, pix2) in enumerate(pixels2):
    #     if i == j: continue
    #   if pix2[0] - pix[0] > 5: 
    #     right.append(j)
    #   if pix[0] - pix2[0] > 5:
    ...
    for (fn, idx), template in templates_items:
      ...
      #   if not "geometry" in q_type:
      #     geometry = False
      # if not geometry and "geometry" in fn: continue
      if args.verbose:
      ...
      if len(ts) > 0 and not "<" in ts[0] and not ">" in ts[0]:
        ...
        #   if tc[0] == fn:
        #     template_counts[tc][0] += 1
        #   if tc[1] == idx:
        #     template_counts[tc][1] += 1
    ...
    # with open(os.path.join(args.new_scene_dir, new_scene_fn), 'w') as f:
    #     print('Writing output to %s' % new_scene_fn)
    #     json.dump({
    #         # 'info': scene_info,
