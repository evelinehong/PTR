...
def add_object2(name, loc, theta=0):
  """
  Load an object from a file. We assume that in the directory object_dir, there
  is a file named "$name.blend" which contains a single object named "$name"
  that has unit size and is centered at the origin.
  ...
  """
  files = []
  with bpy.data.libraries.load("./tmp_depth2/"+name+".blend") as (data_from, data_to):
    for n in data_from.objects:
      if name in n:
        files.append({'name': n})

  bpy.ops.wm.append(directory="./tmp_depth2/"+name+".blend"+"/Object/", files = files)
...
def add_object3(name, loc, rot1, normals, theta=0.0, mode="ground"):
  ...
  files = []
  with bpy.data.libraries.load("./tmp9/"+name+".blend") as (data_from, data_to):
    for n in data_from.objects:
      if name in n:
        files.append({'name': n})

  bpy.ops.wm.append(directory="./tmp9/"+name+".blend"+"/Object/", files = files)
  ...
  if mode == 'support':
    ...
    z = -minz + normals[-1][-1][0] + normals[-1][-2][2] + 0.0025
    if minzz != 10000.0 and sr < 0.7:
      z = -minzz + normals[-1][-1][0] + normals[-1][-2][2] + 0.0025
